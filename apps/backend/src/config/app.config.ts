export default () => ({
  ...
  storage: {
    backend: process.env.STORAGE_BACKEND ?? (process.env.NODE_ENV === 'development' ? 'local' : 'gcs'),
    localRoot: process.env.LOCAL_STORAGE_ROOT ?? 'storage',
    gcsBucket: process.env.GCS_BUCKET ?? '',
    gcsProjectId: process.env.GCS_PROJECT_ID ?? '',
  },
  uploadPolicy: {
    imageMaxBytes: Number(process.env.UPLOAD_MAX_IMAGE_BYTES ?? 25 * 1024 * 1024),
    pdfMaxBytes: Number(process.env.UPLOAD_MAX_PDF_BYTES ?? 50 * 1024 * 1024),
    otherMaxBytes: Number(process.env.UPLOAD_MAX_OTHER_BYTES ?? 100 * 1024 * 1024),
  },
});
